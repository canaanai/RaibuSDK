...
public abstract class Effect {
    ...
    private boolean isFromSurfaceTexture = true;
    ...
    private int inputImageTextureHandle = -1;
    ...
    public void setTextureId(int textureId, boolean isFromSurfaceTexture){
        this.mTextureId = textureId;
        this.isFromSurfaceTexture = isFromSurfaceTexture;
    }
    ...
    private void loadShaderAndParams(String vertex, String fragment) {
        ...
        if (!isFromSurfaceTexture)
            inputImageTextureHandle = GLES20.glGetUniformLocation(mProgram, "sTexture");
        loadOtherParams();
    ...
    protected void createEffectTexture() {
        if(CameraHolder.instance().getState() != CameraHolder.State.PREVIEW) {
            return;
        }

        GlUtil.checkGlError("initFBO_S");
        GLES20.glGenFramebuffers(1, mFboId, 0);                                     //创建fbo的id
        GLES20.glGenRenderbuffers(1, mRboId, 0);                                    //创建rbo的id
        GLES20.glGenTextures(1, mTexId, 0);                                         //创建纹理id

        GLES20.glBindRenderbuffer(GLES20.GL_RENDERBUFFER, mRboId[0]);               //操作id为mRboId[0]的rbo
        GLES20.glRenderbufferStorage(GLES20.GL_RENDERBUFFER,                        //设置rbo信息
                GLES20.GL_DEPTH_COMPONENT16, mWidth, mHeight);

        GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, mFboId[0]);                 //操作id为mFboId[0]的fbo
        GLES20.glFramebufferRenderbuffer(GLES20.GL_FRAMEBUFFER,                     //设置rbo到fbo
    ...
    public void draw(final float[] tex_mtx) {
        ...
        if (isFromSurfaceTexture)
            ...
            GLES20.glUniform1i(inputImageTextureHandle, 0);
